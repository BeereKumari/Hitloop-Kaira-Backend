"""
Audio Interview API — LiveKit Cloud token generation + evaluation storage.

Endpoints:
  POST /api/interview/audio/token    — generate LiveKit room token for candidate
  POST /api/interview/audio/end      — store transcript + evaluation after session
  GET  /api/interview/audio/attempts — list previous audio interview attempts
"""
...
router = APIRouter(prefix="/api/interview/audio", tags=["Audio Interview"])
...
async def get_audio_token(
    ...
    Also checks attempt limits and creates a pending session record.
    """
    ...
    if user_id != "anonymous":
        ...
        if not schedule or not schedule.get("audio"):
            raise HTTPException(status_code=403, detail="Audio interview has not been scheduled by your recruiter yet.")
        
        chat_config = schedule.get("chat")
        if not chat_config or chat_config.get("decision") != "shortlist":
            raise HTTPException(
                status_code=403,
                detail="Prerequisite not met: You must complete and pass the Chat interview before starting the Audio interview."
            )

        audio_config = schedule["audio"]
        scheduled_time = audio_config.get("scheduled_time")
        if scheduled_time:
            now_str = datetime.utcnow().isoformat()
            if now_str < scheduled_time:
                raise HTTPException(status_code=403, detail=f"Your Audio interview is scheduled for {scheduled_time}. It has not started yet.")
        deadline = audio_config.get("deadline")
        if deadline:
            now_str = datetime.utcnow().isoformat()
            if now_str > deadline:
                raise HTTPException(status_code=403, detail=f"The deadline for this Audio interview has passed ({deadline}). You can no longer start it.")
        req.complexity = audio_config.get("complexity", "medium")
        req.num_questions = int(audio_config.get("num_questions", 8))
        req.max_attempts = int(audio_config.get("max_attempts", 3))
    ...
    if completed >= req.max_attempts:
        raise HTTPException(
            status_code=403,
            detail=f"Audio interview attempt limit reached ({completed}/{req.max_attempts}). Contact your recruiter."
        )

    attempt_number = completed + 1
    session_id     = str(uuid.uuid4())
    room_name      = f"kaira-audio-{session_id[:12]}"

    # ── Load candidate context for the agent ──
    ...
    # Fetch previous chat interview evaluation for context enrichment
    prev_eval_doc = await db["interview_evaluations"].find_one(
        {"user_id": user_id, "interview_type": {"$ne": "audio"}},
        sort=[("created_at", -1)],
    )
    prev_eval = prev_eval_doc.get("evaluation", {}) if prev_eval_doc else {}
    ...
    candidate_data = {
        ...
        "prev_chat_strengths":    prev_eval.get("demonstrated_strengths", []),
        "prev_chat_improvements": prev_eval.get("areas_for_improvement", []),
    }
    ...
    # ── Dispatch the Kaira agent to the room ──
    # Tells LiveKit Cloud to send a job to our registered kaira-audio agent worker
    ...
    async with LiveKitAPI(url=lk_http_url, api_key=LIVEKIT_API_KEY, api_secret=LIVEKIT_API_SECRET) as lk:
        await lk.agent_dispatch.create_dispatch(
            CreateAgentDispatchRequest(agent_name="kaira-audio", room=room_name)
        )
...
async def end_audio_session(
    ...
    """
    Called when interview ends (by agent or candidate).
    Receives transcript, generates evaluation, stores in MongoDB + vectorizes.
    """
    user_id = str(current_user["_id"]) if current_user else "anonymous"
    ...
    # ── Build transcript string for evaluation ──
    transcript_str = "\n\n".join([
    ...
    evaluation = await _generate_audio_evaluation(
        transcript=transcript_str,
        candidate_name=candidate_data.get("name", "Candidate"),
        applied_role=candidate_data.get("role", "Software Engineer"),
        num_questions=len([t for t in req.transcript if t.get("speaker") in ("Kaira", "AI")]),
    ...
    await db["interview_sessions"].update_one(
        {"session_id": req.session_id},
        {"$set": {"status": "completed", "audio_url": req.audio_url, "updated_at": now}},
    )
    ...
    try:
        ...
        if current_user and current_user.get("email"):
            asyncio.create_task(notify_stage_completed(
                ...
                stage_name="Audio Interview",
                attempt_num=session.get("attempt_number", 1),
                score=final_score
            ))
    except Exception as email_err:
        print(f"Error sending audio completion email: {email_err}")
...
@router.get("/attempts")
async def list_audio_attempts(
    current_user: Optional[dict] = Depends(get_current_user_optional),
):
    """List all previous audio interview attempts for the current user."""
    user_id = str(current_user["_id"]) if current_user else "anonymous"
...
# ─── Audio Evaluation Prompt ──────────────────────────────────────────────────

AUDIO_EVAL_PROMPT = """You are a senior technical recruiter evaluating an audio interview.
Analyze the transcript and return a valid JSON object with this exact schema:

{
  "final_score": 7.5,
  "recommendation": "Hire",
  "overall_summary": "3-4 sentence evaluation of technical depth, communication quality, and structured thinking.",
  "demonstrated_strengths": ["Specific strength with evidence"],
  "areas_for_improvement": ["Specific area for development"],
  "topic_evaluations": [
    {"topic": "System Design", "score": 8, "feedback": "Detailed assessment"}
  ],
  "communication_score": 8,
  "technical_explanation_score": 7,
  "structured_thinking_score": 8,
  "response_quality_score": 7,
  "technical_depth_score": 7,
  "problem_solving_score": 8,
  "hiring_notes": "Private panel notes: key risks or standout moments."
}
...
async def _generate_audio_evaluation(
    transcript: str,
    candidate_name: str,
    applied_role: str,
    num_questions: int,
    ...
    user_content = (
        ...
        f"Computed Score (use this as final_score): {computed_score}/10\n"
    ...
    def _call():
        response = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": AUDIO_EVAL_PROMPT},
