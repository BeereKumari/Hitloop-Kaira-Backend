...
from app.services.behaviour_service import generate_behaviour_questions, generate_behaviour_evaluation

router = APIRouter(prefix="/api/behaviour", tags=["Behavioural Assessment"])

class SubmitBehaviourRequest(BaseModel):
...
async def get_behaviour_questions(
    ...
    if not schedule or not schedule.get("ai_fluency"):
        raise HTTPException(
            status_code=403,
            detail="Behaviour round is locked. You must complete the AI Tool Fluency assessment and be shortlisted first."
        )
    
    ai_conf = schedule["ai_fluency"]
    if not ai_conf.get("analysis_completed") or ai_conf.get("decision") != "shortlist":
        raise HTTPException(
            status_code=403,
            detail="Behaviour round is locked. You must complete the AI Tool Fluency assessment and be shortlisted first."
        )
    ...
    if not schedule.get("behaviour"):
        raise HTTPException(
            status_code=403,
            detail="Behaviour round has not been scheduled by your recruiter yet."
        )
        
    beh_conf = schedule["behaviour"]
    
    # Enforce scheduled start time
    scheduled_time = beh_conf.get("scheduled_time")
    if scheduled_time:
        now_str = datetime.utcnow().isoformat()
        if now_str < scheduled_time:
            raise HTTPException(
                status_code=403,
                detail=f"Your Behavioural assessment is scheduled for {scheduled_time}. It has not started yet."
            )
            
    # Enforce deadline
    deadline = beh_conf.get("deadline")
    if deadline:
        now_str = datetime.utcnow().isoformat()
        if now_str > deadline:
            raise HTTPException(
                status_code=403,
                detail=f"The deadline for this Behavioural assessment has passed ({deadline})."
            )

    # 3. Load or generate detailed questions
    cached = await db["behaviour_questions"].find_one({"user_id": user_id})
    if cached:
        return cached["questions"]

    ctx = await _load_candidate_context(user_id)
    complexity = beh_conf.get("complexity", "medium")
    num_questions = int(beh_conf.get("num_questions") or 5)
    
    questions = await generate_behaviour_questions(
    ...
    await db["behaviour_questions"].insert_one({
        "user_id": user_id,
        "questions": questions,
...
@router.post("/submit")
async def submit_behaviour(
    req: SubmitBehaviourRequest,
    current_user: Optional[dict] = Depends(get_current_user_optional),
):
    if not current_user:
        raise HTTPException(status_code=401, detail="Authentication required.")
    ...
    if not schedule or not schedule.get("ai_fluency"):
        raise HTTPException(
            status_code=403,
            detail="Behaviour round is locked. Complete AI Tool Fluency and get shortlisted first."
        )
    ...
    if not schedule.get("behaviour"):
        raise HTTPException(
            status_code=403,
            detail="Behaviour round has not been scheduled."
        )
    ...
    max_attempts = int(beh_conf.get("max_attempts", 3))
    ...
    # 3. Fetch questions
    questions_doc = await db["behaviour_questions"].find_one({"user_id": user_id})
    if not questions_doc:
        raise HTTPException(
            status_code=400,
            detail="Behavioural questions not initialized. Fetch questions first."
        )
    ...
    evaluation_result = await generate_behaviour_evaluation(
        questions=questions,
        answers=req.answers,
    ...
    try:
        ...
        if current_user and current_user.get("email"):
            ...
            asyncio.create_task(notify_stage_completed(
                ...
                stage_name="Behavioural",
                attempt_num=completed_count,
                score=evaluation_result.get("overall_score", 0)
            ))
    except Exception as email_err:
        print(f"Error sending behaviour completion email: {email_err}")
